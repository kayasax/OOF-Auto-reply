...
7. Also inspect the owned automation's browser mode. If `browserHeadless` is not explicitly `false`, explain that Outlook account selection and MFA require a visible Scout-managed browser, then offer the in-place `browserHeadless: false` migration for explicit confirmation. Never claim an updated release alone changes this persisted scheduler field.
