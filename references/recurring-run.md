...
9. Run `node "<resourceDir>\scripts\compute-period.cjs"` with today's local date, confirmed working days and hours, eligible OOF dates, and holiday dates. Use its JSON output as authoritative. Do not calculate these values mentally. Render complete expected bodies from `returnDate` and `messageVariant` now.
...
11. Never report no write required until the calendar result and final Outlook snapshot establish coverage end, nearest eligible OOF block or `none`, traversed non-working dates, expected start, expected end, return date, message variant, and exact body equality.
