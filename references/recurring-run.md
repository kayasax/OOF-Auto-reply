...
6. Run `node "<resourceDir>\scripts\outlook-discovery.cjs" --mode=scheduled`. Scheduled discovery skips Work Hours. Use only the confirmed timezone, working days, and working hours from the configuration. Never use `playwright-browser_run_code`. If discovery fails, stop with `OOF_RUN_BLOCKED outlook=unread`; never substitute host browser exploration or stale Outlook state.
