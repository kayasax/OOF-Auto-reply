...
2. Take one `playwright-browser_snapshot`. If it contains only the Microsoft logo or a loading shell while the URL remains the exact Automatic Replies URL, perform one bounded stabilization only: wait up to 10 seconds for the Automatic Replies switch or Save or Enregistrer button, then take one final snapshot. Do not navigate again, reload, inspect another tab, click a category, or repeat the wait. For every other page state, classify the first snapshot immediately.
