...
4. The classified snapshot is usable only when it contains the Automatic Replies switch, scheduled-period controls, both date fields, internal editor, external-send toggle, external editor, and one commit button labelled Save, Enregistrer, OK, or Apply. These labels are equivalent localized page variants. Never reject an otherwise complete page because it exposes OK instead of Save. If any required control is absent after the one permitted loading-only stabilization, or the final URL is the mailbox, inbox, login shell without a recognized authentication control, Microsoft-logo-only, loading-only, or another settings page, stop with `OOF_RUN_BLOCKED outlook=unread`. Do not click category tabs, retry the URL, switch tabs, infer hidden controls, or infer autosave.
...
3. Set the switch, scheduled period, start, end, external-send toggle, and bodies only from calculator and renderer output, then use `playwright-browser_click` exactly once on the commit ref labelled Save, Enregistrer, OK, or Apply from the original snapshot. Prefer the enabled OK button when the page shows OK and Cancel. If no supported commit ref was present, stop before the first edit with `OOF_RUN_BLOCKED outlook=unread`. Never infer that this page autosaves.
...
`OOF_BROWSER_RECEIPT fast_path=1 discovery_navigations=1 discovery_snapshots=<1|2> loading_stabilizations=<0|1> verification_navigations=<0|1> verification_snapshots=<0|1> tab_operations=0 category_clicks=0 run_code=0 elapsed_ms=<N>`
