...
3. If Microsoft account selection, sign-in, or MFA is visible during a headless run, do not wait or attempt credentials. Follow the recurring contract's authentication migration: set only the owned automation's `browserHeadless` field to `false`, then stop with `OOF_RUN_BLOCKED outlook=authentication-required next=visible`. That next visible execution is an authentication-recovery probe only. After direct user authentication exposes the Automatic Replies controls, restore `browserHeadless: true` and stop before reading calendar, comparing bodies, or writing Outlook. If authentication is not completed, stop safely and leave the automation visible for another authentication attempt.
...
2. For each rich-text body that differs: click its editor, press `Control+A` with `playwright-browser_press_key`, then enter the renderer's canonical plain text once with `playwright-browser_type`. Never append or edit sentence by sentence. Treat this automation path as plain text. Preserve every literal URL in the canonical text exactly, including `https://github.com/kayasax/OOF-Auto-reply`. Do not emit HTML tags and do not attempt `Control+K` rich-text link creation.
