...
3. Read public holidays for the current and next year from `holiday_cache`. Fetch a country-year from Nager.Date only when that exact cache entry is absent, then persist only holiday dates.
4. Compute explicit local-time calendar bounds before any calendar call: `calendarStart` is today at 00:00 and `calendarEndExclusive` is 22 calendar days later at 00:00. Pass both bounds to the host calendar-read capability and follow pagination until complete. Never use a convenience query whose range defaults to today, an agenda view, or a call with a missing start or end. The interval must include today, every day through the 21st day ahead, and therefore all of that final day. Example for Saturday, 2026-07-18 in `Europe/Paris`: start `2026-07-18T00:00:00+02:00`, exclusive end `2026-08-09T00:00:00+02:00`; this includes all of Saturday, 2026-08-08. Outlook Automatic Replies settings are not calendar evidence. If the calendar read fails, is unavailable, is today-only, or does not cover the interval, stop with `OOF_RUN_BLOCKED calendar=range-incomplete start=<calendarStart> end=<calendarEndExclusive>`; never assume there is no upcoming leave and never continue to Outlook.
...
Before comparing with Outlook, record the exact requested start and exclusive end, returned coverage end, the nearest eligible OOF block or `none`, the traversed non-working dates, and the computed expected start, end, return date, and message variant. A zero-event result is valid only when it came from the complete bounded calendar read above.
