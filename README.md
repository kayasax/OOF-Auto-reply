...
To update, download and extract the new asset, then import its clean `OOF-auto-reply` folder and choose **Replace**. Never import a git clone. Releases before 0.2.8 stored the full run contract inside Scout, so upgrading one of those installations requires one interactive skill run to migrate the existing automation to the stable bootstrap. After that migration, future replacements are picked up automatically because every scheduled run reads the current installed contract file. If Scout contains multiple matching automations, the skill stops without changing them and asks you to keep one.
